...
description: Receitas prontas pros cenários mais comuns em plugins — comando simples, sessão por chat, setup com notificação, dados persistentes, i18n, comandos admin, lembrete agendado.
sidebar:
  order: 24
---
...
> Essa receita usa um `Map()` manual, mas hoje já existe [`ctx.session`](/docs/api/ctx-session/)
> 🧪 fazendo esse mesmo lock por chat dentro do kernel — vale considerar em vez de reimplementar.
