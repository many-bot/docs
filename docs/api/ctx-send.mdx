...
await ctx.send.gif("/tmp/anim.gif", "legenda");               // convertido pra mp4 via ffmpeg
...
As opções aceitas por cada método (`text`, `image`/`video`/`gif`, `audio`, `poll`, `sticker`/`file`)
estão reunidas em [ctx — opções e assinaturas](/docs/api/ctx-options/#ctxsend).

> `ctx.send.gif()` (e `ctx.msg.reply.gif()`) converte o arquivo automaticamente para `mp4` via
> `ffmpeg` antes de enviar — o WhatsApp não aceita GIF de verdade como mídia.
